...
public class MagnatiPosIntegrationLibraryModule extends ReactContextBaseJavaModule {
  ...
  @ReactMethod
  public void triggerPosInitialize(String uuid, Integer connectionTimeout, Integer transactionTimeout, Integer settlementTimeout, boolean enableTrace, Callback myFailureCallback, Callback mySuccessCallback) {
    executor.execute(new Runnable() {
      ...
      public void run() {
        try {
          ...
          map.putString("code", "XXXX");
          myFailureCallback.invoke(map);
        }
      }
    });
  }
  ...
  @ReactMethod
  public void startTransactionMode(Callback myFailureCallback, Callback mySuccessCallback) {
    executor.execute(new Runnable() {
      @Override
      public void run() {
        try {
  ...
  @ReactMethod
  public void getAuthorization(Integer amount, String tid, String receiptNo, Integer timeoutInSec, Callback myFailureCallback, Callback mySuccessCallback) throws Exception {
    executor.execute(new Runnable() {
      @Override
      public void run() {
        try {
...
//                promise.resolve(map);
          ...
          try {
            ...
            WritableMap map = Arguments.createMap();
            map.putBoolean("success", false);
            map.putString("message", "POS Connection Timed out");
            map.putString("code", "1111");
            // promise.resolve(map);
...
//            promise.resolve(map);
  ...
  @ReactMethod
  public void stopTransactionMode(Callback myFailureCallback, Callback mySuccessCallback) {
    executor.execute(new Runnable() {
      @Override
      public void run() {
        try {
  ...
  @ReactMethod
  public void getTIDMID(Integer timeoutInSec, Callback myFailureCallback, Callback mySuccessCallback) throws Exception {
    executor.execute(new Runnable() {
      @Override
      public void run() {
        try {
          ...
          Log.d("Timeout in sec..", " " + timeoutInSec);

          ExecutorService singleThreadExecutor = Executors.newSingleThreadExecutor();
          final Future<String> handler = singleThreadExecutor.submit(new Callable() {
            @Override
            public String call() throws Exception {
              VFIMembers var10000;
              String pLicense = "";
              ...
              try {
                ...
                if (!singleThreadExecutor.isTerminated()) {
                  Log.d("cancel non-finished tasks in final", "catched");

                }
                singleThreadExecutor.shutdownNow();
                Log.d("shutdown finished in final", "catched");
